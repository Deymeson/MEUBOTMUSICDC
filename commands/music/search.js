...
module.exports = {
    name: 'search',
    description: 'procurar uma faixa',
    voiceChannel: true,
    options: [
        {
            name: 'song',
    ...
    async execute({ client, inter }) {
        ...
        .setDescription(`${maxTracks.map((track, i) => `**${i + 1}**. ${track.title} | ${track.author}`).join('\n')}\n\nSelecione a escolha entre **1** e **${maxTracks.length}** ou **e** ⬇️`)
        .setTimestamp()
        ...
        collector.on('collect', async (query) => {
            ...
            if (!value || value <= 0 || value > maxTracks.length) return inter.followUp({ content: `Resposta inválida, tente um valor entre **1** e **${maxTracks.length}** ou **cancele**... tente novamente! ❌`, ephemeral: true });
        ...
        collector.on('end', (msg, reason) => {
            if (reason === 'time') return inter.followUp({ content:`A pesquisa expirou ${inter.member}... tente novamente! ❌`, ephemeral: true })
        });
    },
};